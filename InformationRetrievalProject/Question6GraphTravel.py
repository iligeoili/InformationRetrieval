...
plt.axis('off')
plt.savefig('network_degree_centrality_no_labels50_with_out_labesNEW.png')
plt.show()
...
# Define the function to print similarity paths
def print_long_similarity_paths_corrected(G, node_labels):
    for node in G.nodes():
        node_label = node_labels[node]
        print(f"Paths and Similarities from '{node}' (Node {node_label}):")
        for target in G.nodes():
            if node != target and nx.has_path(G, node, target):
                ...
                # Check if the path length is more than 2
                if len(path) > 2:
                    path_description = ""
                    for i, step in enumerate(path):
                        step_label = node_labels[step]
                        # For the first node in the path
                        if i == 0:
                            path_description += f"Node {step_label} ({step}) to "
                        elif i < len(path) - 1:
                            similarity = G[path[i-1]][step]['weight']
                            path_description += f"Node {step_label} ({step}) -> "
                        else:
                            path_description += f"Node {step_label} ({step})"
                    print(path_description)
        print("\n")  # New line for readability

# After defining the corrected function, call it to print the paths correctly:
print_long_similarity_paths_corrected(G, node_labels)
